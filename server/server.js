...
const httpDevServer = () =>
  http.createServer(app).listen(HTTP_PORT, () => {
    console.log(
      `Insecure HTTP server (${NODE_ENV}) listening on port ${HTTP_PORT}`
    )
  })
...
const sslDevServer = () =>
  https
    ...
    .listen(HTTPS_PORT, () => {
      console.log(
        `Secure HTTPS server (${NODE_ENV}) listening on port ${HTTPS_PORT}`
      )
    })
...
const sslProdServer = () =>
  https
    ...
    .listen(HTTPS_PORT, () => {
      console.log(
        `Secure Express server (${NODE_ENV}) listening on port ${HTTPS_PORT}`
      )
    })
